...
void afficherBoutique(User &joueur){
    int choix=0;
    int carte=0;
    std::string name;
    ...
    Creature *zelda=new Creature(15, "Zelda", "Breath of wild", "Charge Attaque", 10, 12, zeldaAttaque1, zeldaAttaque2,20);
    Creature *nathan=new Creature(16, "Nathan Drake", "Knife throwin", "Gun shot", 10, 12, nathanAttaque1, nathanAttaque2,30);
    Creature *sonic=new Creature(10, "Sonic", "Homing attaque", "Spin dash", 10, 12, sonicAttaque1, sonicAttaque2,15);
    Creature *mario=new Creature(8, "Mario", "Smash Meteor", "Fire Ball", 10, 12, marioAttaque1, marioAttaque2,12);
    Creature *cloud=new Creature(25, "Cloud Strife", "Ame Brave", "Meteorain", 10, 12, cloudAttaque1, cloudAttaque2,25);
    ...
    std::cout<<joueur.getNom()<<" ,voulez vous acheter une ou plusieurs carte ?"<<std::endl;
    std::cin>>choix;
    if (choix) {

        do {
            std::cout<<"Vous avez "<<joueur.getArgent()<<" pieces a disposition"<<std::endl;
            std::cout << "Quelle carte voulez vous acheter ?" << std::endl;
            std::cin >> carte;
            carte -= 1;
            if (boutique[carte]->getPrix() > joueur.getArgent()) {
                ...
                joueur.setArgent(0 - (boutique[carte]->getPrix()));
                /// Il faut creer une fonction qui poermet d'ajouter la carte a la collection du joueur et de la memoriser en memoire
            }
...
void acheter(User &joueur1,User &joueur2){
    ...
    do {
        std::cin >> name;
    if (name == joueur1.getNom()) {
        afficherBoutique(joueur1);
    } else if (name == joueur2.getNom())
        afficherBoutique(joueur2);

    std::cout << "Veuillez saisir un nom de joueur correct" << std::endl;
    } while (name != joueur1.getNom() && name != joueur2.getNom());
